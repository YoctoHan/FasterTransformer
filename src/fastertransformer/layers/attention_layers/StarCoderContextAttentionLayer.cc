...
#include "src/fastertransformer/layers/attention_layers/StarCoderContextAttentionLayer.h"
...
template<typename T>
void StarCoderContextAttentionLayer<T>::forward(TensorMap*                output_tensors,
                                          ...
                                          const AttentionWeight<T>* attention_weights)
{
    ...
    FT_CHECK_WITH_INFO(attention_type != AttentionType::FUSED_PADDED_MHA,
                       "StarCoder Context FUSED_PADDED_MHA is not supported !");
...
template<typename T>
StarCoderContextAttentionLayer<T>::StarCoderContextAttentionLayer(size_t           max_batch_size,
                                                      size_t           max_seq_len,
                                                      size_t           head_num,
...
template<typename T>
StarCoderContextAttentionLayer<T>::StarCoderContextAttentionLayer(StarCoderContextAttentionLayer<T> const& attention_layer):
...
template<typename T>
StarCoderContextAttentionLayer<T>::~StarCoderContextAttentionLayer()
{
...
template<typename T>
void StarCoderContextAttentionLayer<T>::allocateBuffer()
{
    FT_CHECK(false);
}

template<typename T>
void StarCoderContextAttentionLayer<T>::allocateBuffer(size_t batch_size, size_t seq_len, bool allocate_qk_buf)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void StarCoderContextAttentionLayer<T>::freeBuffer()
{
...
template class StarCoderContextAttentionLayer<float>;
template class StarCoderContextAttentionLayer<half>;
#ifdef ENABLE_BF16
template class StarCoderContextAttentionLayer<__nv_bfloat16>;
#endif
