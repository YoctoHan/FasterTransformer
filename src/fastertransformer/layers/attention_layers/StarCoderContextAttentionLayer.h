...
template<typename T>
class StarCoderContextAttentionLayer: public BaseAttentionLayer<T> {
private:
    // buffer handling
...
protected:
    ...
    // int8_mode_ == 1 for weight quantized only gemm for StarCoder
    // int8_mode_ == 2 for SmoothQuant O3 (per tensor scales)
    const int int8_mode_ = 0;

public:
    StarCoderContextAttentionLayer(size_t           max_batch_size,
                             size_t           max_seq_len,
                             size_t           head_num,
    ...
    StarCoderContextAttentionLayer(StarCoderContextAttentionLayer<T> const& attention_layer);

    virtual ~StarCoderContextAttentionLayer();
