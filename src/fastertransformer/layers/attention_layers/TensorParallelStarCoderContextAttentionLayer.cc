...
#include "src/fastertransformer/layers/attention_layers/TensorParallelStarCoderContextAttentionLayer.h"
#include "src/fastertransformer/utils/nvtx_utils.h"

namespace fastertransformer {

template<typename T>
void TensorParallelStarCoderContextAttentionLayer<T>::forward(TensorMap*                output_tensors,
                                                        ...
                                                        const AttentionWeight<T>* attention_weights)
{
    ...
    StarCoderContextAttentionLayer<T>::forward(output_tensors, input_tensors, attention_weights);
    ...
    if (do_all_reduce_ && tensor_para_.world_size_ > 1) {
        if (!use_custom_all_reduce_kernel) {
            ftNcclAllReduceSum(attention_out, attention_out, size, tensor_para_, StarCoderContextAttentionLayer<T>::stream_);
        }
        else {
            custom_all_reduce_comm_->customAllReduce(size, StarCoderContextAttentionLayer<T>::stream_);
            output_tensors->at("hidden_features").data = reduce_tensor[0].data;
        }
...
template<typename T>
TensorParallelStarCoderContextAttentionLayer<T>::TensorParallelStarCoderContextAttentionLayer(
    ...
    StarCoderContextAttentionLayer<T>(max_batch_size,
                                max_seq_len,
                                head_num,
                                size_per_head,
...
template<typename T>
TensorParallelStarCoderContextAttentionLayer<T>::TensorParallelStarCoderContextAttentionLayer(
    TensorParallelStarCoderContextAttentionLayer<T> const& attention_layer):
    StarCoderContextAttentionLayer<T>(attention_layer),
...
template class TensorParallelStarCoderContextAttentionLayer<float>;
template class TensorParallelStarCoderContextAttentionLayer<half>;
#ifdef ENABLE_BF16
template class TensorParallelStarCoderContextAttentionLayer<__nv_bfloat16>;
#endif
