...
#include "src/fastertransformer/th_op/star_coder/StarCoderOp.h"

namespace th = torch;
namespace ft = fastertransformer;
namespace torch_ext {

StarCoderOp::StarCoderOp(const int64_t                                   head_num,
    ...
    st_(weights[0].scalar_type())
{
    ...
    printf("\n============================ -DEBUG- ============================\n");
    printf("StarCoderOp::StarCoderOp");
    printf("\n============================= -END- =============================\n");
    ...
    ft::starCoderVariantParams star_coder_variant_params{(float)layernorm_eps,
    ...
    switch (st_) {
        case at::ScalarType::Float:
            ftstar_coder = new FTStarCoder<float>((size_t)head_num,
        ...
        case at::ScalarType::Half:
            ftstar_coder = new FTStarCoder<half>((size_t)head_num,
                                                 (size_t)size_per_head,
                                                 (size_t)inter_size,
                                                 (size_t)layer_num,
                                                 (size_t)vocab_size,
...
StarCoderOp::~StarCoderOp()
{
    delete ftstar_coder;
}

std::vector<th::Tensor> StarCoderOp::forward(th::Tensor                                    input_ids,
                                             ...
                                             th::optional<int64_t>          return_cum_log_probs_opt)  { // optional, can be None 
    
    printf("\n============================ -DEBUG- ============================\n");
    printf("StarCoderOp::forward");
    printf("\n============================= -END- =============================\n");
    ...
    ftstar_coder->forward(input_ids,
                          input_lengths,
                          output_ids,
                          sequence_lengths,
                          cum_log_probs,
...
static auto fasterTransformerStarCoderTHS =
#ifdef LEGACY_THS
    torch::jit::class_<torch_ext::StarCoderOp>("FasterTransformerStarCoderOp")
#else
    torch::jit::class_<torch_ext::StarCoderOp>("FasterTransformer", "StarCoderOp")
#endif
        ...
        .def("forward", &torch_ext::StarCoderOp::forward);