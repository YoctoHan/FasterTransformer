...
#include "src/fastertransformer/models/multi_gpu_star_coder/ParallelStarCoderContextDecoder.h"
...
template<typename T>
void ParallelStarCoderContextDecoder<T>::initialize()
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
    self_attention_layer_ = new TensorParallelStarCoderContextAttentionLayer<T>(max_batch_size_,
                                                                          max_seq_len_,
                                                                          head_num_,
                                                                          size_per_head_,
                                                                          tensor_para_,
...
template<typename T>
void ParallelStarCoderContextDecoder<T>::allocateBuffer()
{
    FT_CHECK(false);
}

template<typename T>
void ParallelStarCoderContextDecoder<T>::allocateBuffer(size_t batch_size, size_t seq_len, bool use_shared_contexts)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void ParallelStarCoderContextDecoder<T>::freeBuffer()
{
...
template<typename T>
bool ParallelStarCoderContextDecoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool ParallelStarCoderContextDecoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool ParallelStarCoderContextDecoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int ParallelStarCoderContextDecoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
ParallelStarCoderContextDecoder<T>::ParallelStarCoderContextDecoder(size_t               max_batch_size,
                                                        size_t               max_seq_len,
                                                        size_t               head_num,
    ...
    layernorm_type_(star_coder_variant_params.layernorm_type),
    activation_type_(star_coder_variant_params.activation_type),
    adapter_inter_size_(star_coder_variant_params.adapter_inter_size),
    has_adapters_(star_coder_variant_params.has_adapters),
...
template<typename T>
ParallelStarCoderContextDecoder<T>::ParallelStarCoderContextDecoder(ParallelStarCoderContextDecoder<T> const& decoder):
...
template<typename T>
ParallelStarCoderContextDecoder<T>::~ParallelStarCoderContextDecoder()
{
...
template<typename T>
void ParallelStarCoderContextDecoder<T>::forward(
    TensorMap*                                            output_tensors,
    const TensorMap*                                      input_tensors,
    const std::vector<ParallelStarCoderDecoderLayerWeight<T>*>* star_coder_decoder_layer_weight)
{
    // input tensors:
    ...
    printf("\n============================ -DEBUG- ============================\n");
    printf("ParallelStarCoderContextDecoder::forward");
    printf("\n============================= -END- =============================\n");
    ...
    if (use_shared_contexts) {
        invokeCompactInputsStarCoder(compact_decoder_features_,
    ...
    for (uint ite = 0; ite < iteration_num; ite++) {
        ...
        for (uint l = 0; l < num_layer_; l++) {
            ...
            ParallelStarCoderDecoderLayerWeight<T>* layer_weight = star_coder_decoder_layer_weight->at(l);
            ...
            //     &self_attention_output_tensors, &self_attention_input_tensors, &layer_weight->MQAWeight);
            ...
            //         self_attn_output_,
            //         nullptr, // layer_weight->attention_weights.attention_output_weight.bias,
            //         nullptr,
            //         nullptr,
            //         nullptr,
            //         nullptr,
            ...
            //                         &ffn_input_tensors,
            //                         &star_coder_decoder_layer_weight->at(l)->after_attention_adapter_weights);
            // }
            ...
            //         layer_weight->post_attn_layernorm_weights.gamma,
            //         layer_weight->post_attn_layernorm_weights.beta,
            //         nullptr, // has_adapters_ ? layer_weight->after_attention_adapter_weights.output_weight.bias :
            ...
            //         nullptr,
            //         nullptr,
            //         nullptr, // const_cast<float*>(layer_weight->mlp_dense_weights.intermediate_weight.scale),
...
template class ParallelStarCoderContextDecoder<float>;
template class ParallelStarCoderContextDecoder<half>;
#ifdef ENABLE_BF16
template class ParallelStarCoderContextDecoder<__nv_bfloat16>;
#endif
