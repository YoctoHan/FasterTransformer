...
#include "src/fastertransformer/kernels/star_coder_kernels.h"
...
template<typename T>
void ParallelStarCoder<T>::initialize()
{

    printf("\n============================ -DEBUG- ============================\n");
    printf("ParallelStarCoder::initialize");
    printf("\n============================= -END- =============================\n");

    star_coder_context_decoder_ = new ParallelStarCoderContextDecoder<T>(0,
                                                                         0,
                                                                         head_num_,
                                                                         size_per_head_,
                                                                         inter_size_,
    ...
    star_coder_decoder_ = new ParallelStarCoderDecoder<T>(0,
                                                          head_num_,
                                                          size_per_head_,
                                                          inter_size_,
                                                          num_layer_,
...
template<typename T>
void ParallelStarCoder<T>::allocateBuffer()
{
    FT_CHECK(false);
}

template<typename T>
void ParallelStarCoder<T>::allocateBuffer(size_t batch_size,
                                    size_t beam_width,
                                    ...
                                    bool   is_return_context_cum_log_probs)
{
    ...
    if (star_coder_variant_params_.has_pre_decoder_layernorm) {
    ...
    if (star_coder_variant_params_.use_attention_linear_bias) {
...
template<typename T>
void ParallelStarCoder<T>::freeBuffer()
{
...
template<typename T>
ParallelStarCoder<T>::ParallelStarCoder(size_t                              max_batch_size,
                            size_t                              max_seq_len,
                            size_t                              max_input_len,
                            size_t                              beam_width,
                            size_t                              head_num,
    ...
    layernorm_eps_(star_coder_variant_params.layernorm_eps),
    ...
    shared_contexts_ratio_(shared_contexts_ratio)
{
    ...
    printf("\n============================ -DEBUG- ============================\n");
    printf("ParallelStarCoder::ParallelStarCoder");
    printf("\n============================= -END- =============================\n");
...
template<typename T>
ParallelStarCoder<T>::ParallelStarCoder(ParallelStarCoder<T> const& star_coder):
    BaseLayer(star_coder),
    head_num_(star_coder.head_num_),
    size_per_head_(star_coder.size_per_head_),
    inter_size_(star_coder.inter_size_),
    num_layer_(star_coder.num_layer_),
    vocab_size_(star_coder.vocab_size_),
    start_id_(star_coder.start_id_),
    end_id_(star_coder.end_id_),
    prompt_learning_start_id_(star_coder.prompt_learning_start_id_),
    prompt_learning_type_(star_coder.prompt_learning_type_),
    beam_search_diversity_rate_(star_coder.beam_search_diversity_rate_),
    layernorm_eps_(star_coder.star_coder_variant_params_.layernorm_eps),
    star_coder_variant_params_(star_coder.star_coder_variant_params_),
    hidden_units_(star_coder.hidden_units_),
    top_k_(star_coder.top_k_),
    top_p_(star_coder.top_p_),
    random_seed_(star_coder.random_seed_),
    temperature_(star_coder.temperature_),
    len_penalty_(star_coder.len_penalty_),
    repetition_penalty_(star_coder.repetition_penalty_),
    tensor_para_(star_coder.tensor_para_),
    pipeline_para_(star_coder.pipeline_para_),
    local_head_num_(star_coder.local_head_num_),
    vocab_size_padded_(star_coder.vocab_size_padded_),
    attention_type_(star_coder.attention_type_),
    int8_mode_(star_coder.int8_mode_),
    custom_all_reduce_comm_(star_coder.custom_all_reduce_comm_),
    enable_custom_all_reduce_(star_coder.enable_custom_all_reduce_),
    shared_contexts_ratio_(star_coder.shared_contexts_ratio_)
{
    initialize();
}

template<typename T>
ParallelStarCoder<T>::~ParallelStarCoder()
{
    delete star_coder_decoder_;
    delete star_coder_context_decoder_;
    delete dynamic_decode_layer_;
    freeBuffer();
}

template<typename T>
void ParallelStarCoder<T>::computeContextCumLogProbs(float*                      cum_log_probs,
                                               ...
                                               const ParallelStarCoderWeight<T>* star_coder_weights)
{
    ...
    if (pipeline_para_.rank_ == pipeline_para_.world_size_ - 1) {
        ...
        invokeGeneralLayerNorm(lp_normed_decoder_output_buf_,
                               ...
                               star_coder_weights->post_decoder_layernorm.gamma,
                               star_coder_weights->post_decoder_layernorm.beta,
                               layernorm_eps_,
                               n_hidden_states,
                               hidden_units_,
                               (float*)nullptr,
...
template<typename T>
void ParallelStarCoder<T>::registerCallback(callback_sig* fn, void* ctx)
{
...
template<typename T>
void ParallelStarCoder<T>::unRegisterCallback()
{
...
template<typename T>
void ParallelStarCoder<T>::forward(std::vector<Tensor>*        output_tensors,
                             ...
                             const ParallelStarCoderWeight<T>* star_coder_weights)
{
    ...
    forward(&output_tensors_map, &input_tensors_map, star_coder_weights);
}

template<typename T>
void ParallelStarCoder<T>::forward(std::unordered_map<std::string, Tensor>*       output_tensors,
                             ...
                             const ParallelStarCoderWeight<T>*                    star_coder_weights)
{
    ...
    printf("\n============================ -DEBUG- ============================\n");
    printf("ParallelStarCoder::forward");
    printf("\n============================= -END- =============================\n");

    
    // printf("\n============================ -DEBUG- ============================\n");
    // int* device_data = input_tensors->at("input_ids").getPtr<int>();
    // size_t count = input_tensors->at("input_ids").shape[1];
    // int* host_data = new int[count];
    // cudaD2Hcpy(host_data, device_data, count);
    // printf("input_ids check\n");
    // for (int i=0;i<count;i++){
    //     printf("input_ids[%2d]=%5d\n", i, host_data[i]);
    // }
    // delete[] host_data;
    // printf("============================= -END- =============================\n");
    ...
    // Yocto : Not activated
    if (star_coder_variant_params_.has_positional_encoding && session_len_ > star_coder_weights->getMaxSeqLen()) {
        FT_LOG_ERROR("The session_len_ (%d) of request is longer than max_seq_len (%d) of embedding table."
                     ...
                     session_len_,
                     star_coder_weights->getMaxSeqLen(),
                     star_coder_weights->getMaxSeqLen());
        session_len_ = star_coder_weights->getMaxSeqLen();
    }
    ...
    printf("\n============================ -DEBUG- ============================\n");
    printf("paramers check:\n");
    printf("batch_size : %d\n", int(batch_size));
    printf("beam_width : %d\n", int(beam_width));
    printf("max_input_length : %d\n", int(max_input_length));
    printf("continue_gen : %d\n", int(continue_gen));
    printf("is_return_context_embeddings : %d\n", int(is_return_context_embeddings));
    printf("initial_step : %d\n", int(initial_step));
    printf("max_context_len : %d\n", int(max_context_len));
    printf("input_tensors->count(\"prompt_learning_task_name_ids\") : %d\n", int(input_tensors->count("prompt_learning_task_name_ids")));
    printf("valid_prompt_inputs : %d\n", int(valid_prompt_inputs));
    printf("use_request_p_prompt_embedding : %d\n", int(use_request_p_prompt_embedding));
    printf("max_request_p_prompt_length : %d\n", int(max_request_p_prompt_length));
    printf("input_tensors->count(\"input_lengths_h\") : %d\n", int(input_tensors->count("input_lengths_h")));
    printf("max_input_without_prompt_length : %d\n", int(max_input_without_prompt_length));
    printf("has_prefix_prompt_ : %d\n", int(has_prefix_prompt_));
    printf("has_p_prompt_tuning_ : %d\n", int(has_p_prompt_tuning_));
    printf("use_loaded_p_prompt_embedding : %d\n", int(use_loaded_p_prompt_embedding));
    printf("has_prefix_soft_prompt_ : %d\n", int(has_prefix_soft_prompt_));
    printf("max_prefix_soft_prompt_length : %d\n", int(max_prefix_soft_prompt_length));
    printf("limit_len_offset : %d\n", int(limit_len_offset));
    printf("gen_len : %d\n", int(gen_len));
    printf("input_tensors->find(\"session_len\") != input_tensors->end() : %d\n", int(input_tensors->find("session_len") != input_tensors->end()));
    printf("session_len : %d\n", int(session_len));
    printf("input_tensors->find(\"memory_len\") != input_tensors->end() : %d\n", int(input_tensors->find("memory_len") != input_tensors->end()));
    printf("memory_len_ : %d\n", int(memory_len_));
    printf("star_coder_variant_params_.has_positional_encoding : %d\n", int(star_coder_variant_params_.has_positional_encoding));
    printf("is_return_context_cum_log_probs : %d\n", int(is_return_context_cum_log_probs));
    printf("!continue_gen : %d\n", int(!continue_gen));
    printf("star_coder_weights->getMaxSeqLen() : %d\n", int(star_coder_weights->getMaxSeqLen()));
    printf("============================= -END- =============================\n");
    ...
    std::cout << "gemm_data_type : " << gemm_data_type << std::endl;
    std::cout << "self_k_cache_shape : ";
    for(const auto &value : self_k_cache_shape) {
        std::cout << value << ' ';
    }
    std::cout << std::endl;
    std::cout << "self_v_cache_shape : ";
    for(const auto &value : self_v_cache_shape) {
        std::cout << value << ' ';
    }
    std::cout << std::endl;
    printf("star_coder_variant_params_.use_attention_linear_bias : %d\n", int(star_coder_variant_params_.use_attention_linear_bias));
    printf("============================= -END- =============================\n");

    // Yocto : Else branch activated
    if (continue_gen) {
        PUSH_RANGE("input tiling and init");
        invokeTileStarCoderInputs(tiled_input_ids_buf_,
        ...
        if (vocab_size_ == vocab_size_padded_) {
            padded_embedding_kernel_ptr_ = star_coder_weights->post_word_embeddings;
        }
        else {
            cudaAutoCpy(padded_embedding_kernel_,
                        star_coder_weights->post_word_embeddings,
        ...
        if (use_loaded_p_prompt_embedding) {
            for (int bs_id = 0; bs_id < batch_size; ++bs_id) {
                ...
                bool                     valid_task_name_id   = task_id < star_coder_weights->prompt_learning_table.size();
                if (valid_task_name_id) {
                    p_prompt_tuning_pair = star_coder_weights->prompt_learning_table.at(task_id);
                }
                else {
        ...
        // handle first step
        // 此处invokeTileStarCoderPromptInputs的功能是将输入复制到tiled_input_ids_buf_内，是否复制prompt由
        if (has_p_prompt_tuning_ || has_prefix_prompt_ || has_prefix_soft_prompt_ || max_input_length > 1) {
            PUSH_RANGE("input tiling and init");
            invokeTileStarCoderPromptInputs(tiled_input_ids_buf_,
                                      tiled_input_lengths_buf_,
            ...
            if (has_prefix_soft_prompt_) {
                ...
                param.embedding_table               = star_coder_weights->pre_word_embeddings;
                param.pos_table                     = star_coder_weights->position_encoding_table;
                ...
                invokeInputIdsEmbeddingLookupPosEncoding(context_decoder_input_buf_,
                                                         output_ids_buf_,
                                                         star_coder_weights->pre_word_embeddings,
                                                         star_coder_weights->position_encoding_table,
                                                         prompt_param,
                                                         tiled_input_ids_buf_,
                                                         1,
                                                         max_input_length,
            ...
            TensorMap decoder_input_tensors(
                {{"decoder_input",
                  Tensor(MEMORY_GPU,
                         ...
                         star_coder_variant_params_.has_pre_decoder_layernorm ? context_decoder_normed_input_buf_ :
                                                                         context_decoder_input_buf_)},
                 {"attention_mask",
                  Tensor(MEMORY_GPU,
                         data_type,
            ...
            star_coder_context_decoder_->forward(
                &decoder_output_tensors, &decoder_input_tensors, &star_coder_weights->decoder_layer_weights);
            return;
    ...
    //                                                          star_coder_weights->pre_decoder_embedding_table,
    //                                                          star_coder_weights->position_encoding_table,
    ...
    //                                            star_coder_weights->pre_decoder_layernorm.gamma,
    //                                            star_coder_weights->pre_decoder_layernorm.beta,
    //                                            layernorm_eps_,
    //                                            batch_size * beam_width,
    //                                            hidden_units_,
    //                                            (float*)nullptr,
    ...
    //                          star_coder_variant_params_.has_pre_decoder_layernorm ?
    ...
    //                 star_coder_variant_params_.has_post_decoder_layernorm ? normed_decoder_output_buf_ : decoder_output_buf_;
    //             if (star_coder_variant_params_.has_post_decoder_layernorm) {
...
template<typename T>
void ParallelStarCoder<T>::sendTensorsToFirstPipelineNode(std::unordered_map<std::string, Tensor>*       output_tensors,
                                                    const std::unordered_map<std::string, Tensor>* input_tensors)
{
...
template<typename T>
void ParallelStarCoder<T>::setOutputTensors(std::unordered_map<std::string, Tensor>*       output_tensors,
                                      const std::unordered_map<std::string, Tensor>* input_tensors,
...
template<typename T>
size_t ParallelStarCoder<T>::getPipelineParallelRank()
{
    return pipeline_para_.rank_;
}

template<typename T>
size_t ParallelStarCoder<T>::getPipelineParallelSize()
{
    return pipeline_para_.world_size_;
}

template<typename T>
size_t ParallelStarCoder<T>::getTensorParallelRank()
{
    return tensor_para_.rank_;
}

template<typename T>
size_t ParallelStarCoder<T>::getTensorParallelSize()
{
    return tensor_para_.world_size_;
}

template<typename T>
size_t ParallelStarCoder<T>::getHiddenUnits()
{
    return hidden_units_;
}

template<typename T>
bool* ParallelStarCoder<T>::getFinishBuffer()
{
    return finished_buf_;
}

template<typename T>
size_t ParallelStarCoder<T>::getStep()
{
    return step_;
}

template class ParallelStarCoder<float>;
template class ParallelStarCoder<half>;
#ifdef ENABLE_BF16
template class ParallelStarCoder<__nv_bfloat16>;
#endif
