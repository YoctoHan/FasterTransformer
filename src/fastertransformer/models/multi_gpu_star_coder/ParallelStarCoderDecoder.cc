...
template<typename T>
void ParallelStarCoderDecoder<T>::initialize()
{
...
template<typename T>
ParallelStarCoderDecoder<T>::ParallelStarCoderDecoder(size_t                              max_batch_size,
                                          size_t                              head_num,
...
template<typename T>
ParallelStarCoderDecoder<T>::ParallelStarCoderDecoder(ParallelStarCoderDecoder<T> const& decoder):
...
template<typename T>
void ParallelStarCoderDecoder<T>::allocateBuffer()
{
    FT_CHECK(false);
}

template<typename T>
void ParallelStarCoderDecoder<T>::allocateBuffer(size_t batch_size)
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
...
template<typename T>
void ParallelStarCoderDecoder<T>::freeBuffer()
{
...
template<typename T>
bool ParallelStarCoderDecoder<T>::isValidLayerParallelId(uint l)
{
...
template<typename T>
bool ParallelStarCoderDecoder<T>::isFirstLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
    return l < num_layer_ && (l == local_num_layer * pipeline_para_.rank_);
}
...
template<typename T>
bool ParallelStarCoderDecoder<T>::isLastLayerParallelId(uint l)
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
int ParallelStarCoderDecoder<T>::getFirstLayerParallelId()
{
    int local_num_layer = (int)(ceil(num_layer_ * 1.0f / pipeline_para_.world_size_));
...
template<typename T>
ParallelStarCoderDecoder<T>::~ParallelStarCoderDecoder()
{
...
template<typename T>
void ParallelStarCoderDecoder<T>::forward(std::unordered_map<std::string, Tensor>*              output_tensors,
                                    ...
                                    const std::vector<ParallelStarCoderDecoderLayerWeight<T>*>* star_coder_decoder_layer_weight)
{
    ...
    printf("\n============================ -DEBUG- ============================\n");
    printf("ParallelStarCoderDecoder::forward");
    printf("\n============================= -END- =============================\n");
...
template class ParallelStarCoderDecoder<float>;
template class ParallelStarCoderDecoder<half>;
#ifdef ENABLE_BF16
template class ParallelStarCoderDecoder<__nv_bfloat16>;
#endif
