...
#include "src/fastertransformer/models/star_coder/star_coder_kernels.h"
...
{
    printf("\n=========================================== invokeCreateCausalMasks ===========================================\n");
    printf("\n max_q_len : %d \n", max_q_len);
    printf("\n max_k_len : %d \n", max_k_len);
    printf("\n batch_size : %d \n", batch_size);

    int q_lens_temp, k_lens_temp;
    {
        int* q_lens_host = new int[batch_size];
        cudaD2Hcpy(q_lens_host, q_lens, batch_size);
        for (int i=0; i < batch_size; i++){
            printf("\n q_lens_host[%d]=%d\n", i, q_lens_host[i]);
        }
        q_lens_temp = q_lens_host[0];
        delete[] q_lens_host;
    }
    {
        int* k_lens_host = new int[batch_size];
        cudaD2Hcpy(k_lens_host, k_lens, batch_size);
        for (int i=0; i < batch_size; i++){
            printf("\n k_lens_host[%d]=%d\n", i, k_lens_host[i]);
        }
        k_lens_temp = k_lens_host[0];
        delete[] k_lens_host;
    }

    createCausalMasks<<<batch_size, 512, 0, stream>>>(mask, q_lens, k_lens, max_q_len, max_k_len);
    {
        T* mask_host = new T[q_lens_temp * k_lens_temp];
        cudaD2Hcpy(mask_host, mask, q_lens_temp * k_lens_temp);
        printf("\n mask :\n");
        for (int i = 0; i < q_lens_temp; i++) {
            for (int j = 0; j < k_lens_temp; j++) {
                printf("%3d", int(mask_host[i * k_lens_temp + j]));
            }
            printf("\n");
        }
    }
