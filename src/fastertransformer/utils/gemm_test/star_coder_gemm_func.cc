                              ...
                              bool  isAppend)
{
    ...
    for (int i = 0; i < gemm_num; ++i) {
        ...
        if ((data_type != FLOAT_DATATYPE && i != 1 && i != 2 && i != 10) || data_type == FP8_DATATYPE) {
            ...
            // for star_coder, computeType & scaleType should be FP32
    ...
    printf("***StarCoder Gemm Testing End***\n");
    return;
}

template void generate_star_coder_gemm_config<float>(int   batch_size,
                                              int   beam_width,
                                              int   max_input_len,
                                              int   head_num,
                                              int   size_per_head,
...
template void generate_star_coder_gemm_config<half>(int   batch_size,
                                             int   beam_width,
                                             int   max_input_len,
                                             int   head_num,
                                             int   size_per_head,
...
#ifdef ENABLE_BF16
template void generate_star_coder_gemm_config<__nv_bfloat16>(int   batch_size,
                                                      int   beam_width,
                                                      int   max_input_len,
                                                      int   head_num,
                                                      int   size_per_head,
...
#ifdef ENABLE_FP8
template void generate_star_coder_gemm_config<__nv_fp8_e4m3>(int   batch_size,
                                                      int   beam_width,
                                                      int   max_input_len,
                                                      int   head_num,
                                                      int   size_per_head,
